...
var AUTH='';var am=location.search.match(/[?&]t=([^&]+)/);if(am)AUTH=am[1];function api(p,opts){var sep=p.includes('?')?'&':'?';var r=fetch(p+(AUTH?sep+'t='+AUTH:''),{headers:{'content-type':'application/json'},...opts});return r.then(function(r){if(!r.ok)return r.text().then(function(b){throw new Error(r.status+' '+b)});return r.json()})}
